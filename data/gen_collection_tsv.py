...
def concat_tsv_files(input_files):
    # Concatenate all TSV files into one and gzip
    output_file = f"msmarco-ru/collection/collection.tsv"
...
if __name__ == "__main__":
    ...
    for i in range(0,12):
        if len(str(i)) == 1:
            pyarrow_file_name = f"msmarco-ru/collection/data-0000{i}-of-00012.arrow"

        elif len(str(i)) == 2:
            pyarrow_file_name = f"msmarco-ru/collection/data-000{i}-of-00012.arrow"

        elif len(str(i)) == 3:
            pyarrow_file_name = f"msmarco-ru/collection/data-00{i}-of-00012.arrow"
    ...
    concat_tsv_files(converted_files)
    ...
    os.remove(f"msmarco-ru/collection/collection.tsv")
    print("Done")
