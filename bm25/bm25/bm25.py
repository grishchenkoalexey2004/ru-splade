...
class BM25Search(BaseSearch): 
    ...
    def __init__(self, index_name: str, hostname: str = "localhost", keys: Dict[str, str] =  {"title": "title", "body":"text"}, language: str = "russian",
        ...
        # создает ElasticSearch-сервер по конфигурации! 
        self.es = ElasticSearch(self.config)
        if self.initialize:
            self.initialise()
    
    # инициализация индекса (удаляем старый + sleep + создаем новый)
    def initialise(self):
    ...
    # поиск в индексе
    # дается корпус в формате {id: {title: title, text: text}}
    # дается словарь запросов в формате {id: query}
    # top_k - количество релевантных документов по каждому запросу! 
